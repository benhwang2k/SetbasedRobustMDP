...
class Tree:
    def __init__(self, depth, parent, dist, action, mdpseq):
        ...
        self.children = []
        self.mdpseq = mdpseq
        if self.depth == len(mdpseq):
            self.proc = mdpseq[self.depth - 1]
        else:
...
def get_policy(mdpseq, budget, horizon, defaultvalue):
    """
    :param horizon: 
    :param mdpseq: 
...
def bellman(depth, proc:mdp):
    ...
    while depth > 0:
        ...
        V_next = {}
        for state in proc.S:
            minAction = list(proc.U)[0]
            minVal = math.inf
            for u in proc.U:
                v = proc.C(state, u) + proc.alpha * (sum([(proc.P(x, state, u) * V[x]) for x in proc.S]))
                if v < minVal:
                    minVal = v
                    minAction = u
            V_next[state] = minVal
